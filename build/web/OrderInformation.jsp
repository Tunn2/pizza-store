        ...
        <div class="container">
            ...
            <table class="table">
                ...
                <form action="HomeServlet" method="POST">
                    ...
                    <td><input type="number" min="1" name="numQuantity" value="<%= cart.get(key).getQuantity()%>" readonly=""></td>
                    <td><%= cart.get(key).getSubTotal()%></td>
                    </tr>
                    <% total += cart.get(key).getSubTotal();%>
                    <% }%>
                    Receiver <textarea type="text" name="txtFullName" ><%= user.getFullName()%></textarea>
                    Phone <input type="text" name="txtPhone" value=<%= user.getPhone()%>>
                    Address <textarea type="text" name="txtAddress" ><%= user.getAddress()%></textarea>
                    <% String isSuccess;
                        try {
                            isSuccess = (String) request.getAttribute("SUCCESS");
                        } catch (Exception ex) {
                            isSuccess = null;
                        }

                        if (isSuccess != null) {%>
                    <font color="red"><%= isSuccess %></font>
                    <%    } else { %>
                    <button type="submit" class="btn btn-primary btn-number" name="action" value="Purchase">
                        Purchase
                    </button>
                    <%}%>
