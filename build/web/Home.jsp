                    ...
                    <nav aria-label="breadcrumb" >
                        <ol class="breadcrumb" style="display: flex; align-items: center;">
                            ...
                            <li class="breadcrumb-item" style="display: flex; justify-content: center; align-items: center">Filter by price <form action="HomeServlet" method="POST" class="form-inline my-2 my-lg-0">
                                ...
                                <% if (isAdmin) { %>
                            <li class="breadcrumb-item"><a href="HomeServlet?action=Insert">Create New</a></li> 
                            <li class="breadcrumb-item"><a href="HomeServlet?action=View+All+Orders">View All Orders</a></li> 
                            <li class="breadcrumb-item"><a href="HomeServlet?action=Report+Page">Report</a></li> 
                            <li class="breadcrumb-item"><a href="HomeServlet?action=Best-Seller">Best-Seller</a></li> 
            ...
            <div class="row">
                ...
                <div class="col-sm-9">
                    <div class="row">
                        ...
                        <% List<PizzaDTO> pList = (List<PizzaDTO>) request.getAttribute("LIST_PIZZA"); %>
                        <%for (PizzaDTO item : pList) {
                                if (item.isIsSale() && item.getQuantity() > 0) {%>
                        <div class="col-12 col-md-6 col-lg-4"  style="margin-bottom: 10px">
                            <div class="card">
                                <img class="card-img-top" src=<%= item.getImage()%> height="200px" alt="Card image cap">
                                <div class="card-body">
                                    <h4 class="card-title show_txt"><a href="#" title="View Product"><%= item.getName()%></a></h4>
                                    <p class="card-text show_txt"><%= item.getPrice()%></p>
                                    <div class="row">
                                        <div class="col">
                                            <a href=<%= isAdmin ? "HomeServlet?action=Remove&id=" + item.getId() : "HomeServlet?action=Detail&id=" + item.getId()%> class="btn btn-danger btn-block" ><%= isAdmin ? "Delete" : "Detail"%> </a>
                                        </div>
                                        <div class="col">
                                            <a href=<%= isAdmin ? "HomeServlet?action=Edit&id=" + item.getId() : "HomeServlet?action=Add+To+Cart&id=" + item.getId()%> class="btn btn-success btn-block"><%= isAdmin ? "Edit" : "Add to cart"%></a>
                                        </div>
